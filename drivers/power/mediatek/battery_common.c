...
static kal_bool mt_battery_100Percent_tracking_check(kal_uint32 bat_vol)
{
	kal_bool resetBatteryMeter = KAL_FALSE;
	static kal_uint32 power_check = 1;
...
static void mt_battery_Sync_UI_Percentage_to_Real(kal_uint32 bat_vol)
{
	static kal_uint32 timer_counter;
	static int usb_status_change;
	static int sync_to_real_tracking_time = SYNC_TO_REAL_TRACKING_TIME;

	if (usb_status_change != BMT_status.charger_exist) {
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
		...
		if (timer_counter == (sync_to_real_tracking_time / BAT_TASK_PERIOD)) {
			BMT_status.UI_SOC--;
			timer_counter = 0;
			sync_to_real_tracking_time = SYNC_TO_REAL_TRACKING_TIME;
		} else {
			timer_counter++;
		}
#else
		...
		if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR)) {
			if((BMT_status.UI_SOC == 100) && (bat_vol < 4400) && (BMT_status.bat_full == KAL_FALSE))
				BMT_status.UI_SOC = 99;
		}
	}
	...
	usb_status_change = BMT_status.charger_exist;
}
...
static void battery_update(struct battery_data *bat_data)
{
	...
	bat_vol = battery_meter_get_battery_voltage(KAL_FALSE);
	...
	if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR)) {
		if (BMT_status.bat_exist) {	/* charging */
			if (BMT_status.bat_vol <= V_0PERCENT_TRACKING) {
				...
				resetBatteryMeter = mt_battery_100Percent_tracking_check(bat_vol);
			}
	...
	if (resetBatteryMeter == KAL_TRUE) {
		...
		mt_battery_Sync_UI_Percentage_to_Real(bat_vol);
	}
...
void BAT_thread(void)
{
	...
	if (battery_meter_initilized == KAL_FALSE) {
		battery_meter_initilized = battery_meter_initial();	/* move from battery_probe() to decrease booting time */
